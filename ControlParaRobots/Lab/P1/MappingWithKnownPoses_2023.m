...
% <https://www.mathworks.com/help/releases/R2021b/nav/ref/occupancymap.html occupancyMap> 
...
% (linea 8) de manera que, en función de la posición inicial del robot dentro 
% del entorno, el mapa a obtener quepa bien dentro de la rejilla: 

map = occupancyMap(25,25,20) %creates an occupancy map with a specified grid resolution in cells per meter. resolution sets the Resolution property.
...
while(1)
    ...
    % siguiente llamada
    pose = getTransform(tftree,'odom' , 'robot0_laser_1', msg_laser.Header.Stamp, 'Timeout', 2);
    ...
    angles = msg_laser.readScanAngles; %puede ser tambien angles = sub_laser.read
