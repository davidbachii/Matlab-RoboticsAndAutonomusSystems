...
%% Introduction
% This example demonstrates an application of the Monte Carlo Localization (MCL) algorithm 
% on amigobot in simulated STDR environment.
% 
% Monte Carlo Localization (MCL) is an algorithm to localize a robot using a particle filter.
% The algorithm requires a known map and the task is to estimate the pose (position and 
% orientation) of the robot within the map based on the motion and sensing of the robot.
% The algorithm starts with an initial belief of the robot pose's probability distribution,
% which is represented by particles distributed according to such belief. These particles 
% are propagated following the robot's motion model each time the robot's pose changes. 
% Upon receiving new sensor readings, each particle will evaluate its accuracy by checking
% how likely it would receive such sensor readings at its current pose. Next the algorithm 
% will redistribute (resample) particles to bias particles that are more accurate. 
% Keep iterating these moving, sensing and resampling steps, and all particles should converge
% to a single cluster near the true pose of robot if localization is successful.
% 
% Adaptive Monte Carlo Localization (AMCL) is the variant of MCL implemented in 
% <docid:nav_ref.bu31hfz-1 |monteCarloLocalization|>. AMCL dynamically adjusts the number 
% of particles based on KL-distance [1] to ensure that the particle distribution converge 
% to the true distribution of robot state based on all past sensor and motion measurements 
% with high probability.
% 
% The current MATLAB® AMCL implementation can be applied to any differential drive robot 
% equipped with a range finder.
% 
% The STDR simulation must be running for this example to work.
% 
...
% In simulation: spawn a simulated AmigoBot inside an office environment in a virtual machine
% In real: connect to AmigoBot In your MATLAB instance on the host computer, run the following commands
% to initialize ROS global node in MATLAB and connect to the ROS master in the virtual machine 
% through its IP address |ipaddress|. Replace '172.28.195.100' with the IP address of your robot 
% in virtual machine or real.

%rosinit(ipaddress);
%% Load the map of the simulation world /real environment
% Load a binary occupancy grid of the office environment in STDR. The map is generated by driving 
% AmigoBot inside the office environment. The map is constructed using range-bearing readings 
% from laser sensor and ground truth poses from STDR or real. Refer to 
% <docid:robotics_examples.example-MappingWithKnownPosesExample Mapping With Known Poses> 
% for a more detailed explanation.
load map_modified_OL25.mat 
show(map_modified);
map = map_modified;
...
while (1)
    ...
    % odompose = sub_localOdom.LatestMessage;
    
    %Crear objeto para almacenar el escaneo LiDAR 2-D
    scans = lidarScan(scan);
    
    % For sensors that are mounted upside down, you need to reverse the order of 
    % scan angle readings using 'flip' function.
